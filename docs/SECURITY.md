# Security notes & pen-test checklist

**Threat model:** the WPF client runs on clinic workstations and is treated as
**untrusted for secrets** — it holds only a short-lived access token (memory)
and talks to the API over TLS. The API server holds all credentials (JWT
secret, provider API keys, Fernet key) and is the only party that ever
contacts AI providers. Postgres holds clinical data; Redis holds only
rate-limit buckets and shared health state (never source of truth). The edge
(nginx) terminates TLS and blocks `/docs`, `/openapi.json`, and `/metrics`
from outside.
...
- `MS_DATABASE__AUTO_CREATE=false` + Alembic migrations as the only schema path.
...
- Rate limits sized: `MS_RATE_LIMIT__AUTH_PER_MINUTE` tight (default 10);
  ...
  shared — the limiter degrades **open** if Redis dies, and the login lockout
  (argon2 + 5-failure lock) remains the second layer.
...
- Provider API keys decrypt in exactly one function
  (`ai_bridge.provider_config_with_secrets`) and never appear in listings,
...
- PHI scrubbing before ANY cloud LLM call (setting
  `MS_LLM__CLOUD__REDACT_PHI_FOR_CLOUD`, default on;
  `backend/api/services/pii_ner.py::RedactionService`).
...
5. **Rate limits:** burst `/api/v1/auth/login` from one IP → 429 with
   Retry-After; general bucket; WS per-user cap (6th concurrent session →
   error frame + close 4400).
...
9. **Dependency supply chain:** pip-audit + gitleaks green in CI; image built
   from the repo Dockerfile only.
...
- Load-test numbers are on the mock STT path only (docs/RELEASE.md); the
